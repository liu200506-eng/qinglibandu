    ...
    for chapter_index, (chapter_name, point_names) in enumerate(COURSE_TREE.items(), 1):
        ...
        for point_index, point_name in enumerate(point_names, 1):
            ...
            if not node:
                ...
                db.add(node)
                created += 1
                print(f"✓ 新建知识点: {point_name}")
            else:
                if node.parent_id is None:
                    node.parent_id = chapter.id
                if lecture and (not node.lecture_text or len(node.lecture_text.strip()) < 50 or "待补充" in node.lecture_text):
                    node.lecture_text = lecture
                    filled += 1
                    print(f"✓ 填充讲义: {point_name} (来自本地文档)")
