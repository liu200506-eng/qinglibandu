...
async def get_subjects(education_level: Optional[str] = None, db: Session = Depends(get_db)):
    query = db.query(Subject).filter(Subject.name == PRIMARY_SUBJECT)
    if education_level:
...
async def get_tree(subject_name: str, education_level: Optional[str] = None,
    ...
    if subject_name != PRIMARY_SUBJECT:
        return {'status': 'error', 'message': '比赛版仅开放计算机网络课程', 'tree': []}
        ...
        try:
            ...
            for chunk in stream_llm(lecture_prompt, system_message=lecture_system):
                if chunk.startswith('[Error]'):
                    raise RuntimeError(chunk.removeprefix('[Error]').strip())
                lecture_text += chunk
