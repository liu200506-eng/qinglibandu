...
class DualRetrieval:
    ...
    def retrieve(self, query: str, top_k: int = 5, filters: Optional[dict] = None) -> List[Dict]:
        ...
        reranked_with_meta = []
        for text, score in reranked[:top_k * 2]:
        ...
        merged_results = self._merge_adjacent_chunks(reranked_with_meta)
        
        return merged_results[:top_k]
    
    def _merge_adjacent_chunks(self, results: List[Dict]) -> List[Dict]:
        if not results:
            return []
        ...
        for source, chunks in results_by_source.items():
            ...
            while i < len(chunks):
                ...
                max_score = current["rerank_score"]
                all_sources = set(current["sources"])
                
                j = i + 1
                while j < len(chunks):
                    next_chunk = chunks[j]
                    next_index = next_chunk.get("chunk_index", 0)
                    current_index = current.get("chunk_index", 0)
                    
                    if next_index == current_index + (j - i):
                        next_text = next_chunk["text"]
                        max_overlap = min(len(merged_text), len(next_text), settings.chunk_overlap + 50)
                        
                        overlap_size = 0
                        for possible_overlap in range(max_overlap, 10, -1):
                            if merged_text.endswith(next_text[:possible_overlap]):
                                overlap_size = possible_overlap
                                break
                        
                        merged_text = merged_text + next_text[overlap_size:]
                        max_score = max(max_score, next_chunk["rerank_score"])
                        all_sources.update(next_chunk["sources"])
                        j += 1
                    else:
                        break
