...
def _transcribe_xfyun(audio_bytes: bytes) -> str | None:
    try:
        import websockets
    except ImportError:
    ...
    async def _recognize():
        async with websockets.connect(auth_url) as ws:
            ...
            while True:
                ...
                if data["code"] != 0:
                    logger.error(f"讯飞ASR错误: {data['code']} - {data['message']}")
                    return
    ...
    try:
        ...
        logger.error(f"讯飞ASR调用失败: {e}")
        return None


def xfyun_transcribe_audio_bytes(audio_bytes: bytes) -> str | None:
    result = [None]
    
    def target():
        result[0] = _transcribe_xfyun(audio_bytes)
    ...
    if thread.is_alive():
        logger.error("讯飞ASR超时")
        return None
    
    return result[0]


def is_xfyun_asr_available() -> bool:
    return bool(settings.XFYUN_ASR_APP_ID and settings.XFYUN_ASR_API_KEY and settings.XFYUN_ASR_API_SECRET)
