      ...
      >
        ...
        <div class="result-content">
          ...
          <div class="result-meta">
            ...
            <span class="sources">来源: {{ result.sources?.join(', ') }}</span>
          </div>
        </div>
      </div>
    </div>
...
async function loadStats() {
  try {
    const data = await aiApi.ragStats()
    stats.value = data
  } catch {}
}
...
async function handleUpload() {
  ...
  try {
    ...
    const data = await aiApi.ragUpload(files)
    if (data.results) {
...
async function handleSearch() {
  ...
  results.value = []
  try {
    const data = await aiApi.ragQuery(query.value, topK.value)
