  ...
  <div class="resource-view">
    <el-card class="search-card" shadow="never">
      ...
      <div style="margin-top:14px;display:flex;align-items:center;gap:14px;flex-wrap:wrap">
        <el-form-item label="学习阶段" label-width="76px" style="margin:0">
          <el-radio-group v-model="educationLevel" class="edu-switch-group" size="large">
            <el-radio-button value="high_school">高中</el-radio-button>
            <el-radio-button value="university">大学</el-radio-button>
          </el-radio-group>
        </el-form-item>
        ...
        >
          ...
          <span>{{ isGenerating ? 'AI 生成中...' : '🧠 AI 生成本章节讲义 / 习题' }}</span>
        </el-button>
      </div>
    </el-card>
        ...
        <el-tabs v-model="activeTab" class="right-tabs">
          <el-tab-pane label="📘 讲义" name="lecture">
            <div v-if="resourcePack" class="resource-pack">
              ...
              <div class="pack-section">
                <h4>📖 核心讲义</h4>
                <div class="lecture-text">{{ resourcePack.lecture }}</div>
              </div>
          ...
          <el-tab-pane label="📝 习题" name="exercise">
            <div v-if="resourcePack && resourcePack.exercises?.length" class="exercise-list">
              ...
              >
                ...
                <div v-if="showAnswer(ex)" class="exercise-answer">✅ 正确答案：{{ correctText(ex) }}</div>
            ...
            <el-empty v-else description="当前知识点还没有习题，请先点「AI 生成本章节讲义 / 习题」生成" />
          </el-tab-pane>
...
const educationLevel = ref<'high_school' | 'university'>('high_school')
...
const pickOption = (ex: any, opt: string, _oIdx: number) => {
  ...
  ex.__answered = true
  ElMessage.success(opt === correctText(ex) ? '✅ 回答正确' : `❌ 答错了 · 正确答案：${correctText(ex)}`)
}
...
const loadSubjectOptions = async () => {
  try {
    ...
    subjectOptions.value = list.map((s: any) => (typeof s === 'string' ? s : s.name)).filter(Boolean)
    ...
    subjectOptions.value = educationLevel.value === 'university'
      ? ['Python程序设计', '机器学习', '人工智能导论', '计算机组成', '编译原理', '高等数学', '线性代数']
      : ['高等数学', '数学', '物理', '化学', '英语', '语文', '历史', '地理']
    selectedSubject.value = subjectOptions.value[0] || ''
...
const loadKnowledgeTreeForSubject = async () => {
  ...
  try {
    ...
    const sectionsByChapter: Record<string, any[]> = {}
    const allNodes: any[] = []
    const chapters = tree.map((ch: any) => {
      const chChildren = ch.children || ch.nodes || []
      chChildren.forEach((c: any) => {
        if (c.type === 'section' || c.children) {
          sectionsByChapter[ch.name] = sectionsByChapter[ch.name] || []
          sectionsByChapter[ch.name].push({
            id: c.id,
            name: c.name,
          ...
          allNodes.push(...(c.children || []))
        } else {
          allNodes.push({ ...c, section_id: ch.id, mastery: perNodeMastery[c.id] || 'pending' })
        }
      })
      const sections = sectionsByChapter[ch.name] || []
      return {
        id: ch.id || ch.name,
        name: ch.name,
...
const handleNodeClick = async (node: any) => {
  ...
  const secId = node.section_id
  const ch = chapterList.value.find(c => c.id === activeChapter.value)
  const sec = ch?.sections?.find((s: any) => s.id === secId)
  const payload = {
  ...
  try {
    const r = await fetch('/api/resource/generate', {
      method: 'POST',
    ...
    resourcePack.value = {
      intro: data.intro || `本节目标：掌握「${node.name}」的核心概念与典型题型。`,
      lecture: data.lecture || data.content || `### ${node.name}\n\n（讲义自动生成中，请稍等片刻...）`,
      flash_cards: (data.flash_cards || []).map((fc: any) => ({
        front: fc.front || fc.question || fc.term || '',
        back: fc.back || fc.answer || fc.definition || ''
      })),
      exercises: (data.exercises || []).map((ex: any) => normalizeExercise(ex))
    }
  } catch (_e) {
    resourcePack.value = defaultResourceFor(node.name)
  }
  ...
  try {
    if (node.id) {
      ...
      if (d.exercises?.length || d.flash_cards?.length) {
        resourcePack.value.exercises = (d.exercises || []).map(normalizeExercise)
        if (!resourcePack.value.flash_cards?.length && d.flash_cards?.length) {
          resourcePack.value.flash_cards = (d.flash_cards || []).map((fc: any) => ({
            front: fc.front || fc.question || fc.term || '',
            back: fc.back || fc.answer || fc.definition || ''
          }))
        }
...
const handleGenerate = async () => {
  ...
  try {
    await handleNodeClick(activeNode.value)
    ElMessage.success('✅ AI 讲义 / 习题 已刷新')
  } finally {
...
const startQuiz = async () => {
  ...
  try {
    ...
    if (!quiz.total) {
      ElMessage.warning('该学科尚未生成题库，先点「AI 生成本章节讲义 / 习题」')
    } else {
      quiz.running = true
    }
  } catch (e) {
