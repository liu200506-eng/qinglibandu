...
const loadKnowledgeTreeForSubject = async () => {
  ...
  try {
    ...
    const chapters = tree.map((ch: any) => {
      ...
      let sections = sectionsByChapter[ch.name] || []
      // 同时兼容“章节→知识点”的两级数据，避免叶子节点被旧的三级解析逻辑丢弃。
...
const handleNodeClick = async (node: any) => {
  ...
  try {
    ...
    const r = await fetch(`/api/db/videos?${params.toString()}`)
    if (!r.ok) throw new Error(`HTTP ${r.status}`)
    const data = await r.json()
    const pack = data.resource_pack || data
    resourcePack.value = {
      intro: pack.intro || `本节目标：掌握「${node.name}」的核心概念与典型题型。`,
      lecture: pack.lecture_text || pack.lecture || pack.content || `### ${node.name}\n\n当前知识点内容尚未生成，请点击右上角“AI生成”后重试。`,
      flash_cards: (pack.flash_cards || []).map((fc: any) => ({
        front: fc.front || fc.question || fc.term || '',
        back: fc.back || fc.answer || fc.definition || ''
      })),
      exercises: (pack.exercises || []).map((ex: any) => normalizeExercise(ex)).filter(Boolean)
    }
  } catch (_e) {
