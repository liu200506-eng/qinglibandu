...
- **自动化测试**：52项测试全部通过（15项集成测试 + 21项工具链测试 + 13项业务测试 + 3项故障注入测试）
...
| 章节 | 名称 | 核心知识点 |
|------|------|------------|
| 第1章 | 概述 | 网络概念、互联网结构、性能指标 |
| 第2章 | 物理层 | 传输媒体、编码调制、信道复用 |
| 第3章 | 数据链路层 | 帧结构、差错检测、MAC协议、PPP |
| 第4章 | 网络层 | IP协议、路由算法、ICMP、IPv6 |
| 第5章 | 传输层 | TCP协议、UDP协议、端口、套接字 |
| 第6章 | TCP拥塞控制 | 慢启动、拥塞避免、快速重传、快速恢复 |
| 第7章 | 应用层 | HTTP、DNS、FTP、SMTP、WebSocket |
| 第8章 | 网络安全 | 对称加密、非对称加密、SSL/TLS、防火墙 |

**知识点先修关系**：概述 → 物理层 → 数据链路层 → 网络层 → 传输层 → TCP拥塞控制 → 应用层 → 网络安全
...
│   │       ├── course.json            # 课程基本信息（8章，28个核心知识点）
│   │       ├── knowledge_tree.json    # 知识树定义（8章，28个核心知识点）
│   │       ├── dependencies.json      # 知识点先修依赖关系
│   │       ├── error_patterns.json    # 错误模式库（8种典型错误）
│   │       ├── question_bank.json     # 题库（80+道题，覆盖核心知识点）
│   │       ├── resources.json         # 学习资源索引
│   │       ├── evaluation_dataset.json # A/B/C/D实验评测数据集（80+题）
│   │       ├── sources.json           # 来源文档清单
...
│   │   ├── test_api.py                # API测试
│   │   ├── test_magicstudy.py         # 业务测试（13项）
│   │   ├── test_knowledge_toolchain.py # 工具链测试（21项）
...
系统支持四组对照实验，基于120道计算机网络题目（覆盖8章28个核心知识点，包含概念题、计算题、协议分析题、代码题和故障诊断题）进行评测，严格控制变量：
...
> **说明**：拥塞控制仿真作为独立功能案例演示，不参与问答准确率对比实验。

### 实验结果

| 评价指标 | A组（LLM） | B组（LLM+RAG） | C组（LLM+RAG+审查） | D组（完整系统） |
|----------|------------|----------------|---------------------|-----------------|
| 事实正确率 | 70% | 80% | 90% | 90% |
| 引用正确率 | - | 70% | 80% | 80% |
| 个性化匹配度 | 30% | 40% | 40% | 80% |
| 平均响应时间 | 1.2s | 2.8s | 3.2s | 3.8s |
...
- **测试数据集**：120道计算机网络题目，覆盖8章28个核心知识点，每题满分5分，共计600分
- **事实正确率**：按知识点评分点计分（每题包含2-3个评分点，满分5分/题），事实正确率=各题事实评分总和÷600×100%
- **引用正确率**：检查回答是否引用正确文档，低于相似度阈值时明确拒答
- **个性化匹配度**：四组统一输入学生画像，按以下量表评分（满分5分/题）：
...
- **评分方式**：回答隐藏组别、随机打乱顺序，由两名非核心开发成员独立评分，存在分歧时由第三人复核
- **评分者一致性**：使用Cohen's Kappa系数评估评分者一致性
- **模型版本**：讯飞星火大模型（v3.0）、BGE-base-zh-v1.5 Embedding、Qdrant v1.8.0

### 原始结果文件

实验原始数据存储于：
- `backend/tests/experiment/questions.json`：120道测试题目及标准答案（含知识点、难度、题型、先修知识、来源元数据）
- `backend/tests/experiment/scoring_rubric.json`：评分标准说明
- `backend/tests/experiment/scores.csv`：各组别详细评分记录
- `backend/tests/experiment/summary.json`：实验汇总数据
- `backend/tests/experiment/run_experiment.py`：实验复现脚本
...
在本次120道计算机网络题目的实验中，加入审查机制后，C组和D组的事实正确率均达到90%；进一步引入学习画像后，D组的个性化匹配度由40%提升至80%。结果表明，审查机制主要改善事实可靠性，学习画像主要改善个性化适配效果。
...
| v1.5.0 | 2026-07 | 知识库治理与安全发布体系：分级校验策略（draft/demo_only/review/published/archived）、来源追溯、人工审核工作流、蓝绿发布基础设施、故障注入测试（52项全通过） |
