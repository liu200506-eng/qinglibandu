...
class RAGEngine:
    ...
    def ingest_document(self, file_path: str, source_name: str = None, metadata: Dict = None) -> Dict:
        file_hash = calculate_file_hash(file_path)
        source_file = source_name or os.path.basename(file_path)
    ...
    def retrieve(self, query: str, top_k: int = 5, subject: str = None) -> List[Dict]:
        filters = {"subject_name": subject} if subject else None
        return self.retrieval.retrieve(query, top_k=top_k, filters=filters)

    def get_stats(self, subject: str = None) -> Dict:
        stats = self.vector_store.get_collection_stats()
        if subject:
            stats["total_points"] = self.vector_store.count_by_filter({"subject_name": subject})
