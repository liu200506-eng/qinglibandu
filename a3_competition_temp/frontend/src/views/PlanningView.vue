...
const strategies = [
  { value: 'weakness', name: '查漏补缺', description: '专注薄弱知识点', icon: '🎯' },
  { value: 'boost', name: '提分冲刺', description: '快速提升分数', icon: '🚀' },
  { value: 'sprint', name: '考试冲刺', description: '考前突击复习', icon: '⏰' },
...
function startTask(task: LearningTask) {
  ...
  localStorage.setItem('planner_pending', JSON.stringify({
    subject: formData.value.subject || '数学',
    title: task.title,
    knowledge: subjects,
    ts: Date.now(),
  }))
