...
function createFetchOptions(options: RequestInit = {}, timeout: number = TIMEOUT_MS): RequestInit {
  ...
  async ragUpload(files: File[], subject = '计算机网络') {
    try {
      ...
      if (!res.ok) {
        ...
        return { status: 'error', message: err.message || '上传失败' }
      }
      return await res.json()
    } catch (e) {
      return { status: 'error', message: '上传超时或失败' }
    }
  },
